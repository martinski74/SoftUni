...
class Program
{
    static void Main()
    {
        ...
        while (bombIndex != -1)
        {
            ...
            {
                resultField.AddRange(field.Skip(bombPower * 2 + 1 + firstBombedIndex + resultField.Count).
                    Take(field.Count - (bombPower * 2 + 1) - (bombIndex - bombPower)).ToList());
            }
            else
            {
                resultField.AddRange(field.Skip(bombPower * 2 + 1 + resultField.Count).
                    Take(field.Count - (bombPower * 2 + 1) - (bombIndex - bombPower)).ToList());
            }
