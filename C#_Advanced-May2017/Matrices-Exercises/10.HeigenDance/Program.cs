...
namespace _10.HeigenDance
{
    class Program
    {
        ...
        {
            int size = 15;
            int[] playerPosition = { size / 2, size / 2 };  // matrix centre
            double[] points = { 3000000, 18500 };           // Heigan, player
            ...
            while (points.Min() > 0)
            {
                ...
                if (isHitBySpell)
                {
                    ...
                    { playerPosition = escapePosition; continue; }
                    // then right
                    ...
                    else // no escape possible => player suffers damage
                    {
                        ...
                        if (spell == "Cloud") hasActiveCloud = true;
                    }
                }
            }
        ...
        {
            if (points[0] <= 0) Console.WriteLine("Heigan: Defeated!");
            else Console.WriteLine("Heigan: {0:f2}", points[0]);
            if (spell == "Cloud") spell = "Plague " + spell;
            if (points[1] <= 0) Console.WriteLine("Player: Killed by {0}", spell);
            else Console.WriteLine("Player: {0:f0}", points[1]);
        ...
        public static bool IsInsideMatrix(int[] position)
        {
            int size = 15;
            for (int i = 0; i < 2; i++)
                if (position[i] < 0 || position[i] >= size) return false;
            return true;
        }
