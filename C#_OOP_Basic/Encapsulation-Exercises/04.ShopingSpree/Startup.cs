    ...
    public class Startup
    {
        public static void Main()
        {
            ...
            try
            {
                ...
                {
                    var persInfo = pair.Split('=');
                    var person = new Person(persInfo[0], decimal.Parse(persInfo[1]));
                    people.Add(person);
                }

                foreach (var pair in allProducts)
                {
                    var prodInfo = pair.Split('=');
                    var product = new Product(prodInfo[0], decimal.Parse(prodInfo[1]));
